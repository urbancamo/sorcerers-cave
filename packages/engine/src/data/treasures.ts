...
// kit INVENTORY (Idol and Crypt/Gems are HEAVY treasure, 25 kg each; the rest are artifacts). Kept
// as a SEPARATE table rather than appended onto `TREASURES` itself: the base array's length is
// pinned by `data.test.ts`, which must stay green unmodified (SC-EXT-2). `ALL_TREASURES` below is
// the combined, id-indexed lookup for code that needs to resolve either a base or a kit treasure.
