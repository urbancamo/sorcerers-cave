...
| RB-3-4 | *(added 2026-07-13, amends RB-6-4)* A recorded high score's detail panel MUST offer a "Replay" of that game in its button cluster (beside the log downloads), launching the same viewer via the same `handleReplay` path; an unreplayable recording surfaces its explanation inline instead. Offered only where a handler is threaded (splash modal and solo post-save leaderboard — not multiplayer). | HighScores.tsx:134-153, SplashScreen.tsx:194, GameOverScreen.tsx:153, GameScreen.tsx:144,171 | HighScores.test.tsx:128 › `offers Replay in the detail's button cluster…`; :140 › `explains when a recorded game cannot be replayed…` |
...
| RB-4-6 | *(added 2026-07-13, supersedes the RB-6-5 v1 non-goal)* The transport MUST offer auto-play: Play advances one frame per 1000 ms until Stop is pressed or the last frame is reached (playback then ends by itself and Play is offered again); a manual jump during playback re-arms the timer from the new frame. Play is disabled at the last frame. The cadence MUST exceed the renderer's 550 ms token-move animation so no step is visually swallowed (cave3d.js `refresh` also retargets an in-flight move rather than dropping it). | ReplayView.tsx:34-45 (timer), :76-81 (Play/Stop); cave3d.js:328-338 | ReplayView.test.tsx:104 › `play animates forward at one second per frame and stops at the end`; :119 › `stop halts playback where it is` |
...
| RB-6-4 | **DECIDED (2026-07-10) — splash screen.** The entry point is a "Replay a game" affordance on the splash screen, visually parallel to the existing "Resume a game" code box (SplashScreen.tsx:114). RB-3-1 already encodes this; the in-game "Game log" adjacency is rejected for v1. **AMENDED (2026-07-13):** a second entry point ships from the high-score detail panel (RB-3-4) — a recorded score already publishes its log and code, so "watch it" belongs beside the downloads. |
| RB-6-5 | ~~**Non-goal:** playback timing/auto-advance.~~ **SUPERSEDED (2026-07-13):** auto-play shipped as the planned fast follow — Play/Stop at a fixed 1000 ms per frame (RB-4-6). Still out of scope: variable speed control. |
...
"move i / n" readout, and Play/Stop auto-advance at one second per frame (RB-4-6). Crucially it
