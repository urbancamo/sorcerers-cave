...
> (D) **DECISION** -- a deliberate, documented implementation choice (rationale on record).
> (A) **ASSUMPTION** -- behavior that exists but was never explicitly decided or documented.
> (X) **NOT IMPLEMENTED** -- a rulebook clause the engine deliberately does not model.
> (?) **PENDING** -- explicitly awaiting the designer's ruling.
> (OK) **FAITHFUL** -- the clause is implemented as the rulebook states it.
> (!) **GAP / WARNING** -- a known defect, undocumented behavior, or rough edge.
...
player from winning"* (rulebook sec. Zombies). The risen party:
...
The normative source is `docs/specs/sorcerers-cave-rules.md:575-585` (sec. Zombies),
...
|---|---|---|
| "he cannot win ... can try to keep any other player from winning" | (OK) faithful (spoiler semantics; but see sec. 4.11 on the *interim* score) | whole module |
| "forfeits one turn, then ... resurrected" | (OK) faithful -- (D) charged *at* the rise via `forfeitTurnsOwed = +1` | multi-zombies.ts:257 |
| "the body of the **last creature to die** ... along with any other bodies **in the area**" | (D) simplified: ALL of the seat's corpses rise together at the wipe area (no per-corpse locations exist) | multi-zombies.ts:58-61 |
| "enters an area containing dead creatures, these ... join the party" (absorption) | (X) NOT IMPLEMENTED -- corpses are not modelled on tiles; a rival's dead stay in that rival's array | multi-zombies.ts:62-64 |
| "enters an area containing the Sorcerer, he and any companions join the party" | (X) NOT IMPLEMENTED (same missing corpse/NPC-location model) | multi-zombies.ts:62-64 |
| "moved by the player during his turn" | (OK) faithful | turn flow unchanged |
| "living strangers are indifferent to zombies" | (OK) faithful -- encounters dissolve, working set parks back untested | multi-zombies.ts:191-211 |
| "cannot carry or use treasure" | (OK) faithful -- pre-gate + post-strip + game-sweep strip (three layers) | multi-zombies.ts:99-102, 213-232, 313-337 |
| "will not attack strangers" | (OK) faithful (test AND attack barred) | multi-zombies.ts:104-105 |
| "not affected by Medusa, vipers, or ghouls" | (OK) faithful -- (D) via run-then-undo repair (see sec. 4.5) | multi-zombies.ts:149-189 |
| "will fall down traps unless accompanied by a living dwarf" | (OK) faithful -- a zombie party by construction has no *living* Dwarf, so Traps always apply | multi-zombies.ts:25 |
| "will not cross water" | (OK) faithful -- Deep Pool tiles unenterable; from a Pool doorway only the retrace is walkable | multi-zombies.ts:106-121 |
| "Sorcerer with them: ALL secret doors; otherwise NONE" | (OK) faithful -- overrides the per-seat `knownDoors` gate in both directions | multi-zombies.ts:122-127, multi.ts:476-480 |
| "can form a union with other zombies" | (OK) faithful -- mixed living/zombie proposals never open | multi.ts:409-419 |
| "attack or be attacked by living creatures in the normal way" | (OK) faithful -- PvP fully open (attack affordance kept in UI) | multi-fight.ts, MultiplayerPlay.tsx:268 |
| "no magical power ... only normal physical strength" | (!) **partially faithful** -- background casters are zeroed; an engaged *front-line* caster still adds its MP (see sec. 4.7) | multi-fight.ts:111-116, 439-450 |
| "a zombie which is 'killed' is reanimated after one full turn ... provided the main party is still in the area" | (X) NOT IMPLEMENTED -- (D) a zombie party killed in PvP is terminal for good (needs the missing corpse model) | multi-zombies.ts:65-66 |
| "If the Sorcerer is killed all zombies are annihilated and no more may be created" | (OK) faithful -- fires off ANY seat's kill, union or not | multi-zombies.ts:273-301 |

Trade is not mentioned by the rulebook; (D) **DECISION**: trade with the risen never
...
- (D) **DECISION -- no prompt**: the M7 design sketch had a *"Rise as zombies?"* option
...
- (D) **DECISION -- items**: carried items already spilled to the tile at the wipe
...
- (D) **DECISION -- memory wiped**: the risen party keeps its seat/name/color and map
...
- **The one-turn forfeit** is charged at the rise (`forfeitTurnsOwed + 1`): strict
  ...
  the existing forfeit lockout. (D) Same net timing as the rulebook's "forfeits one
...
- **A game already finished stays finished** -- the last party's fall ends the game;
  zombies raise no curtain calls. (A) ASSUMPTION: the rulebook doesn't address whether
...
- **Water**: a zombie party never steps ONTO a Deep Pool tile; standing at a Pool's
  ...
  walkable -- every other exit "would mean crossing the water". (D) This doorway
...
- **Secret doors**: with the Sorcerer aboard (living, in the risen party -- only
  possible if the seat's own Sorcerer... see (A) below) ALL secret stairs open; without,
  ...
  - (A) ASSUMPTION: `hasSorcerer` checks for a *living* Sorcerer member of the risen
    party. Since the Sorcerer-joins-the-dead clause is not implemented (sec. 3) and the
...
- **Kit terrain**: the Whirlpool's crossing-drag applies to zombies exactly as to the
  ...
  gates only where they touch treasure -- see sec. 4.6.
...
(D) **DECISION**: the strip preserves the items on the tile (nothing is destroyed) --
...
(D) **DECISION**: "parked exactly as a retreat would leave it" -- the creatures remain
...
hazard caused is undone with its victims. (D) **DECISION** (documented enforcement
...
- (A) ASSUMPTION baked into the repair: members are **index-matched** against the
...
| Harpies (6) | **Applies** | theft targets *artifacts*, not "treasure" in the cannot-carry sense -- but see the milestone report's own caveat: *"the one I would most expect you to push back on is Harpies -- 'the dead cannot carry or use treasure' can be read to cover artifacts too, which would make the theft moot rather than applicable."* In practice a zombie never holds an artifact (sec. 4.3), so the applied case is nearly unreachable. |
...
strangers/treasure then park/strip per sec. 4.3-4.4; a Gallery's statues are stone (not
corpses) and never interact; the Lair's stash is treasure (strippable). (D) DECISION:
...
- (!) **KNOWN GAP (undocumented until the kit milestone's review)**: an engaged
...
- (D) **DECISION**: a zombie command wiped in PvP is **terminal for good** -- the
...
- (A) ASSUMPTION: the victor's automatic floor-reclaim of a slain zombie party's
...
- **Unions**: proposals mixing zombie and living seats never open; zombie-with-zombie
  ...
  (A) ASSUMPTION: *nothing* inside union mechanics is zombie-aware beyond the gate --
...
- (A) ASSUMPTION (inferred, not coded): a zombie party can never itself kill the
...
- (X) The rulebook's "Sorcerer joins the dead" clause is not implemented (sec. 3).
...
- **Terminal**: a finally-terminal zombie seat records outcome *wiped*, **score 0**
  (the standard `gs === GS_DEAD` zero rule). (D) documented and tested.
- (!) **UNDOCUMENTED GAP -- the interim score**: while it walks, a risen party's live
...
- **Leaderboards** (as of the 2026-07-28 four-table split): multiplayer tables list
  **escaped seats only**, so zombie terminal rows (all wipes) never appear on any
  leaderboard -- they exist only in the archive. (A) The `highScores` schema carries
  **no zombie marker**; a former-zombie row is distinguishable only by inspecting its
  stored state snapshot.
- Bounty-split scoring (`sorcererSharedWith`) has no zombie path -- see sec. 4.9.

### 4.12 Presentation (UI, notices, log)

- **Lobby**: toggle withdrawn (sec. 1). Guests of a game already carrying the flag see a
  "Zombies [check]" chip; there is no "Zombies --" chip anymore -- the withdrawn option is
  not advertised.
- **In play**: your own risen party loses the Trade/Unite affordances (Attack stays --
  ...
  **Scoreboard** shows "[skull] risen -- a spoiler, out of the running" beside the name
  (while ranking it by its nonzero score, per sec. 4.11).
- (!) **GAP -- the map**: the 3D cave map's party tokens carry **no zombie signal at
...
- (!) **WORDING**: zombie action denials reuse the PvP rejection frame -- a zombie
...
- **Fog-of-war-lite**: no special interplay -- a risen party records seenAreas and is
  ...
  fog. (A) Never explicitly considered.
- **Concurrent exploration**: the forfeit uses the concurrent lockout path; zombies
  ...
  model-independent.
- **Extension kit**: the SC-EXT-33 matrix (sec. 4.5) plus the shared-content generics
  (sec. 4.6). This is the interaction that prompted the UI withdrawal.

## 5. Decision & assumption register

| # | Area | Item | Status | Recorded where |
|---|---|---|---|---|
| D1 | Rise | All corpses rise together at the wipe area (not "last creature + bodies in the area") | (D) decided | multi-zombies.ts:58-61 |
| D2 | Rise | Auto-rise, no "Rise as zombies?" prompt | (D) decided (MVP) | multi-zombies.ts:46-47 vs design spec sec. I-15 UI note |
| D3 | Rise | Borne items lost with the rising flesh; risen carry nothing | (D) decided | multi-zombies.ts:67-68 |
| D4 | Rise | Forfeit charged at the rise (`forfeitTurnsOwed+1`) | (D) decided | multi-zombies.ts:69-71,257 |
| D5 | Corpses | No absorption; no Sorcerer-joins; no PvP re-animation (no corpse-location model) | (X) decided omissions | multi-zombies.ts:62-66; engine-spec.md "deliberate omissions" |
| D6 | Hazards | Immunity via run-then-undo repair (solo reducer frozen) | (D) decided | multi-zombies.ts:19-25 |
| D7 | Trade | No trading with the risen (derived from cannot-carry) | (D) decided | multi.ts:376-381 |
| D8 | Kit | Desertion/Quarrel ignored; Crypt/Harpies apply | (?) **PENDING your ruling** | SC-EXT-33; milestone report sec. 1 |
| D9 | PvP | Zombie command killed in PvP is terminal for good | (D) decided | multi-zombies.ts:65-66 |
| A1 | PvP | Front-line zombie casters still add MP (only backers zeroed) | (!) **defect vs rulebook**, known, unfixed | multi-fight.ts:139-146 vs 439-450; deferred-minor ledger |
| A2 | Scoring | Interim (walking) score is nonzero and ranked on the Scoreboard | (A) undocumented assumption | this report sec. 4.11 |
| A3 | Movement | Deep-Pool doorway: only the retrace is walkable | (A) interpretation of "will not cross water" | multi-zombies.ts:111-121 |
| A4 | Movement | The Sorcerer-aboard secret-door arm is unreachable in play (no join mechanic) | (A) latent code | multi-zombies.ts:87-88,122-127 |
| A5 | Repair | Index-matched member restore assumes stable zombie rosters | (A) fragility note | multi-zombies.ts:151-153 |
| A6 | Unions | Zombie-union internals rely wholly on generic layers; untested beyond formation | (A) coverage gap | this report sec. 4.8 |
| A7 | Game end | Last living seat's wipe ends the game rather than rising | (A) undecided edge | multi-zombies.ts:303-304 |
| A8 | Data | No zombie marker in highScores rows or map tokens | (A) presentation gaps | schema.ts:76-94; CaveCanvas.tsx:14 |
| A9 | Wording | "Battle plan rejected" frame on zombie denials; "rise from the dead" grammar | (A) copy defects | eventNotices.ts:70-74; convex/multiplayer.ts:724 |
...
special (sec. 4.6), zombie-union play beyond formation (A6).
