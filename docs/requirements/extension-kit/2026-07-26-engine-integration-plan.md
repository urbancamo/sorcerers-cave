...
- **TDD:** every task writes failing tests first. Type-level changes need a real type-check for RED/GREEN — **vitest alone does not type-check** (esbuild strips types). Engine/assets packages: `pnpm --filter <pkg> exec tsc --noEmit`. **apps/web: use `pnpm --filter web typecheck` (`tsc -b`) — plain `tsc --noEmit` is a NO-OP there** (`files: []` + project references; verified in Task 2).
