...
| SC-7.2-6 | Ghouls fight: each living member rolls `frontStrength(m,state)+1d6` vs `2+1d6`; emit `combatRoll` (enemy "Ghouls"). If enemy > party the member is slain (`status=3`) and `eyeForsakenByDeath` runs. No surprise. | hazards.ts:76-90 | hazards.test.ts › Ghouls drop heavy treasure and roll against each member |
...
| SC-11-26 | Lost Ruby (id 11, 20 pts, wt 0) — taking it via `takeTreasure` triggers a strength-8 statue fight: `frontStrength + d6` vs `8 + d6`; win claims it (`combatRoll` vs "Statue"), lose slays the fighter (`memberDied`, `statueAroused`). | reduce.ts:323-369 | ruby.test.ts › taking the Lost Ruby fights the statue; › an overwhelming fighter always wins |
