...
| SC-3-21 | Party member status MUST be 0=original, 1=ally, 2=stone(petrified), 3=dead. There is NO party-size cap — the original rules impose no maximum (a friendly group is always added in full); the only natural bound is the finite small pack. | state.ts:17-18 | gap-data-rng.test.ts |
...
| SC-4-19 | A `friendly` test recruits every eligible stranger as an ally (status 1 — no party-size limit), emits `strangersJoined`; a womanless Unicorn stays as a guard (`unicornGuards`), pacifying the area. Cleared strangers → pickup if treasure, else explore. | reduce.ts:457 | reduce.test.ts › a friendly result recruits the strangers as allies; › recruits EVERY stranger regardless of party size |
...
| SC-4-43 | Re-entering a Deep Pool with `dropped` heavy treasure enters a Giant-only `pickup` to reclaim it (only if a living Giant has spare capacity), emitting `treasureReclaimed(count)`; else it stays sunk. See SC-10.2-4/6. | reduce.ts:128 | reduce.test.ts › re-entering WITH a capable Giant recovers; › WITHOUT a Giant leaves it |
...
| SC-6.1-12 | A printed stair-up on a level-1 card is KEPT — per the rules "any stairway leading up from the first level is an exit from the Cave", so several drawn cards (not only the Gateway) can let a party escape via `exitCave`; a DIR_UP *move* stays blocked on level 1 so no level-0 area is created. | map.ts:89-94 | map.test.ts › keeps a stair-up on a freshly drawn level-1 card; reduce.test.ts › a stair-up card DRAWN onto level 1 is a valid cave exit |
...
| SC-6.1-16 | The mirrored (secret-door) stair is a connectivity link, not printed art: recorded in `mirroredStairs` so tile-art selection can exclude it while traversal uses the full card. | map.ts:137,61-62, state.ts:43 | — (renderer contract in apps/web) |
...
| SC-6-7 | Carpet move: teleports one step in `dir` ignoring doors; unexplored → draw (fallback 31), face-up; descending mirrors a stair-up, ascending a stair-down, as `mirroredStairs`; a printed level-1 stair-up is kept (a cave exit); `fellThroughTrap=false`; may not exit the cave (Up disallowed on level 1). | reduce.ts:229-251,656-664 | carpet.test.ts |
...
| SC-7.3-5 | `leaveTreasure` (pickup only): `persistAndExplore` — remaining floor treasure parks to `area.contents` (200+tid) and phase → explore; retrievable later on return, whether the tile is a chamber or a passage (SC-7.3-12). | reduce.ts:417-424,42-67 | reduce.test.ts › leaving treasure parks it and clears the working set |
| SC-7.3-6 | `dropTreasure` (blocked in fight/gameOver): remove `treasure[idx]`. In pickup it lands back on the live floor; else parks on `area.contents` (reclaimed on return — a chamber via `enterChamber`, a passage tile via SC-7.3-12). Dropping the Eye of God (13) increments `curses` and emits `eyeForsaken`. | reduce.ts:467-485 | reduce.test.ts › drops onto the floor; › re-offers treasure dropped in a tunnel when the party returns; › re-offers pickup-dropped so a Giant can clear room; › blocks during a fight; › forsaking the Eye |
| SC-7.3-12 | Re-entering a passage tile (tunnel / Gateway — `!chamber`, and not a Pool/Pit) that has floor treasure parked on `area.contents` (200+tid) reloads it into `state.treasures`, removes those codes, and enters `pickup` so it can be reclaimed. Without this, treasure dropped or left on a passage tile would be stranded, because a passage tile never runs `enterChamber` (which is what reloads a chamber's parked floor). | reduce.ts:165-179 | reduce.test.ts › re-offers treasure dropped in a tunnel when the party returns |
...
| SC-8.4-7 | Friendly: every eligible stranger joins as an ally (`status:1, dragonKills:0, treasure:[]`); `strangersJoined` event. No party-size cap — the group is always added in full (original rules §8). | reduce.ts:457-465 | reduce.test.ts › a friendly result recruits the strangers as allies; › recruits EVERY stranger regardless of party size |
| SC-8.4-8 | After a friendly join with no guard remainder: strangers cleared; treasure remaining → pickup, else `persistAndExplore` (→ explore). | reduce.ts:466-478 | reduce.test.ts › a friendly result recruits the strangers as allies |
...
| SC-10.2-6 | Deep Pool recovery is a **Giant-only, weight-limited pickup**: while on a Pool tile, `legalActions`/`takeTreasure` offer a take only to living Giants (multiple Giants each count); any other member is blocked. Treasure the Giants can't fit sinks back onto `area.dropped` on leaving (not the floor contents). | selectors.ts:97-110, reduce.ts:320-327,33-40 | reduce.test.ts › a Deep Pool recovery is a Giant-only pickup; › treasure a Giant can't fit is left back in the pool |
...
Full engine suite: **348 tests green**. Keep it that way — when a requirement changes, update both its `test` reference here and the test itself (see the repo `CLAUDE.md`).
