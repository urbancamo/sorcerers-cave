...
/**
 * M7 game variants in the lobby: the HOST gets four toggles (Zombies, Fog of war, Concurrent
 * exploration, Extension kit — Task 8, the MP exposure switch); everyone else gets read-only
 * chips. Convex is mocked out — this exercises the pure host/guest rendering.
 */
...
test("the host sees the four variant toggles reflecting the stored state", () => {
  (useQuery as Mock).mockReturnValue(lobbyFixture());
  render(<MultiplayerLobby code="ABCD" onExit={() => {}} />);
  const boxes = screen.getAllByRole("checkbox");
  expect(boxes).toHaveLength(4); // zombies, fog of war, concurrent exploration, extension kit
